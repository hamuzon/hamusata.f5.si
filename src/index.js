...
export default {
  async fetch(request, env, ctx) {
    ...
    // --- Middleware Logic (from functions/_middleware.js) ---
    ...
    if (ENABLED !== 0) {
      ...
      const isAsset = pathname.includes('.') || EXCLUDED_EXTENSIONS.some(ext => pathname.endsWith(ext));
      const isFavicon = pathname === "/favicon.ico";
      const isDomainTarget = hostname.endsWith("hamusata.f5.si");
      const isBot = /bot|googlebot|bingbot|yandex|baidu|duckduckbot|slurp|ia_archiver/i.test(userAgent);

      if (!isFavicon && !isAsset && isDomainTarget && !isBot) {
    ...
    // /countdown (from functions/countdown.js)
    if (pathname === '/countdown') {
        return handleCountdown(request);
    }

    // /r/* (from functions/r.js and functions/r/[[path]].js)
    ...
    // Redirects (from functions/redirects.js)
    ...
    try {
      ...
      if (e instanceof Error && (e.message.includes('could not find') || e.status === 404)) {
          // Try to serve 404.html
          try {
            const notFoundResponse = await getAssetFromKV(
                {
                    request: new Request(new URL('/404.html', request.url), request),
                    waitUntil: ctx.waitUntil.bind(ctx),
                },
                {
                    ASSET_NAMESPACE: env.__STATIC_CONTENT,
...
function handleRedirects(request) {
    const url = new URL(request.url);
...
function handleRickRoll(request) {
    return new Response(
        `<!DOCTYPE html>
<script>
...
async function handleCountdown(request) {
    const url = new URL(request.url);
