...
async function loadLinks(lang = "ja") {
  ...
  try {
    ...
    // sub-lang.json を参照
    const langMap = window.subLang || {}; // 必ず <script src="sub-lang.json"> で読み込んでおく
    ...
    rows.slice(1).forEach(row => {
      ...
      // 翻訳がある場合は langMap から取得
      if (langMap[lang] && langMap[lang][title]) {
        const map = langMap[lang][title];
        title = map.title ? map.title : title;
        description = map.desc ? map.desc : description;
      }
      ...
      if (link) {
        ...
        a.textContent = langMap[lang] && langMap[lang]["link_view"] ? langMap[lang]["link_view"] : "見る / View";
        card.appendChild(a);
      }
...
// ページ読み込み時に実行
document.addEventListener("DOMContentLoaded", () => loadLinks("ja"));
